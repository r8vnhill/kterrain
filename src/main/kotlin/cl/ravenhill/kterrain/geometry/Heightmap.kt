package cl.ravenhill.kterrain.geometry
...
class Heightmap(detail: Int) {
  ...
  private fun divide(x0: Int, y0: Int, x1: Int, y1: Int, spread: Float = 1f) {
    ...
    map[midX][midY] =
      bound((map[x0][y0] + map[x0][y1] + map[x1][y0] + map[x1][y1]) / 4 + (r.nextFloat() - 0.5f) * spread)
    ...
    for (i in 0 until map.size - 1)
      for (j in 0 until map.size - 1) {
        ...
        if (!vertexMap.containsKey("v($i,${j + 1})"))
          vertexMap["v($i,${j + 1})"] = Vector3f(
              i / floorScale, map[i][j + 1] * upScale,
              (j + 1) / floorScale
          )
        val v2 = vertexMap["v($i,${j + 1})"]
        if (!vertexMap.containsKey("v(${i + 1},$j)"))
          vertexMap["v(${i + 1},$j)"] = Vector3f(
              (i + 1) / floorScale, map[i + 1][j] * upScale,
              j / floorScale
          )
        val v3 = vertexMap["v(${i + 1},$j)"]
        if (!vertexMap.containsKey("v(${i + 1},${j + 1})"))
          vertexMap["v(${i + 1},${j + 1})"] = Vector3f(
              (i + 1) / floorScale,
  ...
  private fun bound(n: Float, lowerBound: Float = 0f, upperBound: Float = 1f) =
    max(lowerBound, min(n, upperBound))
