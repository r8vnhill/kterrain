...
import cl.ravenhill.kterrain.controller.Camera
...
class Shader(vertexPath: String, fragmentPath: String) {
  ...
  fun setMVP(camera: Camera) {
    // Sets the model, view and projection matrices
    setMatrix("modelMatrix", false, camera.modelMatrix.get(BufferUtils.matrixBuffer))
    setMatrix("viewMatrix", false, camera.viewMatrix.get(BufferUtils.matrixBuffer))
    setMatrix("projMatrix", false, camera.projMatrix.get(BufferUtils.matrixBuffer))
  }
}